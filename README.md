...
> - Packet header and content have a character pointer set to the beginning of the packet buffer and advanced to a particular protocol header by the size in bytes. The header is then mapped to a relevant header structure through casting to a protocol specific structure *(ICMP or ARP)*
#### │ Signature Detection:
> - Each Snort rule is converted into a `BPF filter` iteratively storing them within memory
...
> - BPF filter expressions are applied to a packet, it is executed using a kernel level virtual machine (VM) which iterates through a series of instructions, pushing and popping values from the stack as needed and then returning a non-zero value if the packet matches the filter, and zero if it does not. The eval algorithm simulates a generalised coverage of the acyclic control flow graph *(arp.src=XXX)*
#### │ Storage:
> - `SQLite3` database is used for the project to record and store any IOC packets identified as a match with a BPF expression.  
> - Packet information once extracted is bound to string value placeholders in SQLite prepared statements for one final transaction. Inserting into the database is done once all packet information has been recovered; a collaboration of SQL queries avoids deadlocks but maintains steady control of order as to perform simultaneous data presentation
